...
function getRows(duty) {
    ...
    rows.push(["Freight", `${duty.fr} (${duty.calculateFreightPercent()}%)`]);
    rows.push(["Misc", `${duty.misc}`]);
    rows.push(["BCD(%)", `${duty.bcd_rate}%`]);
    rows.push(["INR Value", duty.inr]);
    rows.push(["Insurance", `${duty.insurance} (1.125%)`]);
    rows.push(["Total", duty.total]);
    rows.push(["BCD", duty.bcd]);
    rows.push(["SW", duty.sw]);
    rows.push(["GST", duty.gstAmt]);
    rows.push(["Duty Payable Amount", `${duty.amt} (${duty.calculateDutyInPercent()}%)`]);
    return rows;
}
