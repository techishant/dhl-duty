...
function calculateValues() {
    ...
    billsList[currentDuty].misc = parseFloat(miscValue.value);
    ...
    billsList[currentDuty].calulateMiscDutyValue();
...
function flipOutputs() {
    ...
    freightFinalValue.innerText = `₹${billsList[currentDuty].fr} (${billsList[currentDuty].calculateFreightPercent()}%)`;
    ...
    amtValue.innerText = `₹${billsList[currentDuty].amt} (${billsList[currentDuty].calculateDutyInPercent()}%)`;
...
function flipForm() {
    ...
    let g = billsList[currentDuty].gst;
    for (i = 0; i < GSTval.length; i++) {
        if (parseFloat(GSTval[i].value)==g) {
            GSTval[i].checked=true;
            break;
        }else{
            GSTval[i].checked=false;
        }
    }
